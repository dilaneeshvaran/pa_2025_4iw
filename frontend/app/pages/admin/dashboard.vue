  ...
  <div class="space-y-6">
    <div>
      ...
      <p class="text-gray-600 dark:text-gray-400">
        Vue d'ensemble de la plateforme MediCote
      </p>
    </div>
    ...
    <UiCard>
      ...
      <div v-else class="grid gap-4 sm:grid-cols-4">
        <div class="rounded-lg bg-gray-50 p-4 text-center dark:bg-gray-950">
          <p class="text-3xl font-bold text-gray-900 dark:text-gray-100">
            {{ dashboard?.appointmentStats.total ?? 0 }}
          </p>
          <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Total</p>
        </div>
        <div
        ...
        <div class="rounded-lg bg-red-50 p-4 text-center dark:bg-red-950/40">
          <p class="text-3xl font-bold text-red-700 dark:text-red-300">
            {{ dashboard?.appointmentStats.cancelled ?? 0 }}
          </p>
          <p class="mt-1 text-sm text-red-600 dark:text-red-400">Annulés</p>
        </div>
        <div
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      <UiCard>
        ...
        <div v-if="loading" class="animate-pulse space-y-2">
          <div
            v-for="i in 6"
            :key="i"
            class="h-8 rounded bg-gray-200 dark:bg-gray-700"
          />
        </div>
        <div v-else class="space-y-2">
          <div
            ...
            :key="item.month"
            class="flex items-center justify-between rounded-lg bg-gray-50 px-4 py-2 dark:bg-gray-950"
          >
            <span
            ...
            >
            <span
              class="rounded-full bg-orange-100 px-3 py-0.5 text-sm font-semibold text-orange-700 dark:bg-orange-900/40 dark:text-orange-300"
            >
              {{ item.count }}
            </span>
          </div>
      ...
      <UiCard>
        <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-gray-100">
          <Stethoscope
            class="mr-2 inline h-5 w-5 text-green-600 dark:text-green-400"
          />
        ...
        <div v-else class="space-y-2">
          ...
          >
            ...
            >
            <span
              class="rounded-full bg-green-100 px-3 py-0.5 text-sm font-semibold text-green-700 dark:bg-green-900/40 dark:text-green-300"
            >
              {{ item.count }}
            </span>
          </div>
    ...
    <div class="grid gap-6 lg:grid-cols-2">
      <UiCard>
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
            <CreditCard
              class="mr-2 inline h-5 w-5 text-red-500 dark:text-red-400"
            />
            Abonnements impayés
          </h3>
        ...
        <div v-if="loading" class="animate-pulse space-y-3">
          <div
            v-for="i in 3"
            :key="i"
            class="h-12 rounded bg-gray-200 dark:bg-gray-700"
          />
        </div>
        ...
        <div v-else class="space-y-2">
          <div
            ...
            :key="sub.id"
            class="flex items-center justify-between rounded-lg border border-gray-100 px-4 py-3 transition-colors hover:bg-gray-50 dark:border-gray-800 dark:hover:bg-gray-800"
          >
            <div class="min-w-0 flex-1">
      ...
      <UiCard>
        <div class="mb-4 flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">
            <AlertTriangle
              class="mr-2 inline h-5 w-5 text-orange-500 dark:text-orange-400"
            />
