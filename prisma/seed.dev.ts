...
function generateFakeTOTPSecret(): string {
  return getEnv('SEED_TOTP_SECRET');
}
...
async function main() {
  ...
  await prisma.systemSetting.createMany({
    data: [
      ...
      },
      {
        ...
        value: { providers: ['Orange Money', 'MTN Mobile Money', 'Moov Money'] },
      },
    ],
  });
  ...
  const specialties = await prisma.specialty.createMany({
    data: [
      { name: 'Médecine Générale', description: 'Soins de santé primaires', icon: '🩺' },
      { name: 'Cardiologie', description: 'Maladies du cœur et système cardiovasculaire', icon: '❤️' },
      { name: 'Dermatologie', description: 'Maladies de la peau', icon: '🔬' },
      { name: 'Pédiatrie', description: 'Santé des enfants', icon: '👶' },
      { name: 'Gynécologie', description: 'Santé reproductive féminine', icon: '👩‍⚕️' },
      ...
      { name: 'Orthopédie', description: 'Maladies des os et articulations', icon: '🦴' },
  ...
  for (const data of patientData) {
    const user = await prisma.user.create({
      data: {
        email: data.email,
        password: await hashPassword(getEnv('PATIENT_PASSWORD')),
        role: 'PATIENT',
        status: 'ACTIVE',
        emailVerified: new Date(),
        twoFactorEnabled: false,
  ...
  for (const data of practitionerData) {
    const user = await prisma.user.create({
      data: {
        email: data.email,
        password: await hashPassword(getEnv('PRACTITIONER_PASSWORD')),
        role: 'PRACTITIONER',
        status: 'ACTIVE',
        emailVerified: new Date(),
        twoFactorEnabled: false,
  ...
  const staffUser = await prisma.user.create({
    data: {
      ...
      password: await hashPassword(getEnv('STAFF_PASSWORD')),
      role: 'STAFF',
      status: 'ACTIVE',
      emailVerified: new Date(),
      twoFactorEnabled: false,
  ...
  const medicalRecord = await prisma.medicalRecord.create({
    data: {
      ...
      examination: 'Tension artérielle: 130/85 mmHg, Auscultation cardiaque normale',
  ...
  await prisma.message.createMany({
    data: [
      ...
      },
      {
        senderId: patients[0].id,
        recipientId: practitioners[0].id,
        content: 'Est-ce que je peux prendre le médicament le soir au lieu du matin?',
        status: 'READ',
        readAt: new Date(),
      },
      {
  ...
  await prisma.review.create({
    data: {
      ...
      appointmentId: completedAppt.id,
      rating: 5,
      comment: 'Excellent médecin, très à l\'écoute. Consultation très professionnelle.',
    },
  });
  ...
  const campaign = await prisma.campaign.create({
    data: {
      ...
      message: 'Chers patients, n\'oubliez pas de vous faire vacciner contre la COVID-19. Des doses sont disponibles dans tous nos centres.',
      status: 'ACTIVE',
    ...
    data: [
      {
        ...
        filePath: '/documents/patients/' + patients[0].id + '/rapport_consultation_2025_01.pdf',
        fileSize: 245000,
      ...
      },
      {
        ...
        filePath: '/documents/patients/' + patients[0].id + '/analyse_sanguine_2025_01.pdf',
        fileSize: 128000,
        mimeType: 'application/pdf',
        isEncrypted: true,
      },
      {
        ...
        filePath: '/documents/practitioners/' + practitioners[1].id + '/templates/ecg_result_2025.pdf',
        fileSize: 350000,
        mimeType: 'application/pdf',
        isEncrypted: true,
      },
  ...
  const teleconsultationSession = await prisma.teleconsultationSession.create({
    data: {
      ...
      roomName: 'Consultation Dermatologique - ' + new Date(nextWeek).toLocaleDateString('fr-FR'),
      status: 'SCHEDULED',
    ...
    data: [
      {
        ...
        processedAt: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000 + 3600000), // 1 hour after request
        completedAt: new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000 + 7200000), // 2 hours after request
  ...
  await prisma.dataDeletionRequest.create({
    data: {
      ...
      status: 'CANCELLED',
      reason: 'Je souhaite supprimer mon compte et toutes mes données personnelles.',
      ...
      confirmationToken: 'cancelled_token_' + Math.random().toString(36).substring(7),
  ...
  console.log(` Appointments: ${appointments.length + 1} (+ 1 teleconsultation)`);
